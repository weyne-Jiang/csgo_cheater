...
//
//		//初始化玩家键值表
//		_pPlayerController = PlayerHelper::getInstence();
//
//		//初始化透明窗口、D3D驱动
//		_pDrawHelper = DrawHelper::getInstence();
//        _pDrawHelper->createWindows("Valve001", "Counter-Strike: Global Offensive - Direct3D 9");
...
//{
//	return Point3D::distance(_pPlayerController->getPlayer("player0")->location, target);
//}
//
//// 计算自瞄角度
...
//{
//	//更新数据
//	auto res = _pPlayerController->updatePlayer();
//	if (res == errorType::readErr)
//	{
//		cout << "读取错误\n";
//	}
//    _pPlayerController->updateAngle();
//    _pPlayerController->updateMatrix();
//
//	//获取数据
//	auto playerMap = _pPlayerController->getPlayers();
//	auto self = _pPlayerController->getPlayer("player0");
//	auto selfCamp = self->camp;
//
//	string target;
//	float minDist = 9999;
...
//	{
//		//自身判断
//		if (player.second->isSelf == true)
//			continue;
//
//		//阵营判断
//		if (player.second->camp == selfCamp)
//			continue;
//
//		//血量判断
//		if (player.second->blood <= 0 || player.second->blood > 100)
//			continue;
//
//		//绘制
...
//		//需要屏幕角度转换为屏幕坐标
//		//Point2D point = calcuAngle(self->location, player.second->location);
...
//
//		//计算距离选择最近敌人
//		float dist = distance(player.second->location);
//		if (dist < minDist)
//		{
//			minDist = dist;
//			target = player.first;
...
//	if (target.length() > 0)
//	{
//		Point2D aimAngle = calcuAngle(self->location, playerMap[target]->location);
//
//		//cout << "瞄准角度 ： " << aimAngle << endl;
//		//printf("瞄准玩家 ：%s\n", target.c_str());
//		//printf("玩家血量 ：%d\n", playerMap[target]->blood);
//		//printf("最小距离 : %f\n", minDist);
//
//		//自瞄角度写入屏幕坐标
//		if (getMouseL())
